...
if(detect_system() == 'wiiu')
{
	session_start();
	$_SESSION['client'] = 'wiiu';
	include('redirect.php');
	exit();
}
...
for($m = 0; $m < $numbah; $m++)
{
	...
	if($play_id[$m] == $player_id)
	{
		...
		//echo 'pos is '.$my_left_pos . ','.$my_top_pos.'<br>';
...
.main_screen
{
	position: absolute;
	top: 0px;
	left: <?php if($mapsize_width < 320){ /*echo (160 - ($mapsize_width / 2));*/}else{/*echo 0;*/}?>0px;
...
?>
	...
	if(document.activeElement.id != 'buttoninput')
	{
		if(new_scrolltop == last_scrolltop && in_trade == false && crafting == false && windows_open == 0 && magic_value >= 20)
		{
			// B-button is pressed
			document.getElementById('buttoninput').focus();
	...
	last_scrolltop = document.getElementById('msgs').scrollTop;
<?php
}
?>
}
...
<div id='msgs' style='border: 1px solid black; background-color: brown; overflow-x: hidden; overflow-y: scroll; visibility: visible; z-index: 999998; position: absolute; top: 110px; left: 200px; height: 70px; width: 118px; font-size: 10px;'>Loading messages...</div>
...
<div id='dialogue_box' style='position: absolute; top: 120px; left: 0px; border: 2px outset brown; width: 312px; padding: 2px; height: 54px; background-color: orange; z-index: 999999; font-size: 10px; visibility: hidden;'>
...
<script>document.getElementById('maincontain').scrollLeft = <?php echo $viewport_left - 140;?>; document.getElementById('maincontain').scrollTop = <?php echo $viewport_top - 76;?>;</script>
<script>setInterval("document.body.scrollTop=0;",1);</script>
</html>