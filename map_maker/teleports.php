...
{
	$czone = str_replace("teleport","'teleport'",$datazonesplit[$m]);
	$czone = str_replace("shop","'shop'",$czone);
	$checkzonesplit = explode(",",$czone);
	if(count($checkzonesplit) == 7 && $checkzonesplit[4] == "'teleport'")
	{
...
function point_it(event){
	...
	if(click == 1)
	{
		...
		else
		{
			...
			var toroom = parseInt(window.prompt('To which room\'s ID should this spot teleport?'));
			var tospot = parseInt(window.prompt('To which spot in the room?'));
			...
			newdiv.innerHTML = "<span class='delete_zone' oNclick='delete_zone("+zone_id+");'>X</span> "+toroom+","+tospot;
			document.getElementById('pointer_div').appendChild(newdiv);
			click = -1;
			zones[zone_id] = new Array(curx,cury,newx,newy,'teleport',toroom,tospot);
		}
	}
	click++;
}
...
function done()
{
	for(var o in zones)
	{
		...
		if(currzone != '')
		{
			...
			var bc = currzone[3];
			var type = currzone[4];
			var toroom = currzone[5];
			var tospot = currzone[6];
			if(document.getElementById('sendzones').value != '')
			{
				document.getElementById('sendzones').value += ';';
			}
			document.getElementById('sendzones').value += lc+','+tc+','+rc+','+bc+','+type+','+toroom+','+tospot;
		}
	}
...
for($a = 0; $a < count($preset_zones); $a++)
{
	...
	$toroom = $zsplit[5];
	$tospot = $zsplit[6];
	echo "<div id='zone_$a' style='width: " . ($rc - $lc) . "px; height: ". ($bc - $tc) . "px; left: ".$lc."px; top: ".$tc."px; position: absolute; background-color: yellow; opacity: 0.5; border: 1px solid black;'><span class='delete_zone' oNclick='delete_zone($a);'>X</span> $toroom,$tospot</div>";
}
?>
...
<div id='not_picture'>
Set the teleport areas.<br>Press the upper left spot, then the bottom right spot to create a teleport area.<br>
<br>
...
<form action='saveteleports.php?id=<?php echo $id;?>' method='post'>
