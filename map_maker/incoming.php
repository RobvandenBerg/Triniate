...
for($m = 0; $m < count($datazonesplit); $m++)
{
	...
	if(!empty($czone))
	{
		$splitczone = explode(".",$czone);
		$preset_zones[count($preset_zones)] = $splitczone[0] . ",'" . $splitczone[1] . "'";
	}
}

//echo count($preset_zones) . "<hr>";
...
function point_it(event){
	...
	if(click == 0)
	{
		var curx = parseInt(pos_x);
		var cury = parseInt(pos_y);
		var walkdir = window.prompt('What side will the player point to when dropped? Type left, right, up or down');
		if(walkdir == 'left')
		{
			var realdir = 'left';
		}
		...
		var savedir = 'sprite_' + realdir;
			...
			for(i = 0; i < zone_id; i++)
			{
				if(check_existence('zone_number_'+i))
				{
					nzone++;
					document.getElementById('zone_number_'+i).innerHTML = nzone;
				}
			}
			...
			newdiv.innerHTML = "<span class='delete_zone' oNclick='delete_zone("+zone_id+");'>X</span> <span id='zone_number_"+zone_id+"'>"+isin+"</span>, " + realdir;
			document.getElementById('pointer_div').appendChild(newdiv);
			click = -1;
			zones[zone_id] = new Array(curx,cury,savedir);
...
function check_existence(cplayer)
{
...
function done()
{
	...
	for(var o in zones)
	{
		...
		if(currzone != '')
		{
			...
			document.getElementById('sendzones').value += lc+','+tc+'.'+rc;
		}
	}
...
for($a = 0; $a < count($preset_zones); $a++)
{
	...
	$lc = $zsplit[0];
	$tc = $zsplit[1];
	$sprite = str_replace("sprite_","",str_replace("'","",$zsplit[2]));
	echo "<div id='zone_$a' style='left: ".$lc."px; top: ".$tc."px; position: absolute; background-color: yellow; opacity: 0.5; border: 1px solid black;'><span class='delete_zone' oNclick='delete_zone($a);'>X</span> <span id='zone_number_$a'>$a</span>, $sprite</div>";
}
?>
...
<div id='not_picture'>
Set the incoming areas for this map.<br>Press the spot, then set the facing direction when being dropped in.<br>
<br>
<br>
<input type='button' value='Done' oNclick='done();' id='finish_button' style='display: inline;'>
<div id='submit_div' style='display: none;'>
<form action='saveincoming.php?id=<?php echo $id;?>' method='post'>
